	...
	var socketInit = function () {
		...
		return function (port) {
			...
			socket.onopen = function socketOpen() {
				socket.open = true; //testing mode//open
				global.message = 'either not a valid BT token, or it is already in'; //either not a valid BT token, or it is already in //The Server is testing mode;no play can join!
		...
		function drawPoly(context, centerX, centerY, radius, sides) {
			...
			if (!sides) {
				...
				for (var _i10 = 0; _i10 < 6; _i10++) {
					...
					if (_i10 == 4) {
						_x19 = centerX + radius * Math.cos(1.18 * Math.PI + angle);
						_y = centerY + radius * Math.sin(1.18 * Math.PI + angle);
					}
					if (_i10 == 5) {
						_x19 = centerX + radius * Math.cos(1.58 * Math.PI + angle);
						_y = centerY + radius * Math.sin(1.58 * Math.PI + angle);
					}
				...
				for (var _i11 = 0; _i11 < sides; _i11++) {
					var _theta3 = _i11 / sides * 2 * Math.PI;
					var _x20 = centerX + radius * Math.cos(_theta3 + angle);
					var _y2 = centerY + radius * Math.sin(_theta3 + angle);
			...
			if (source.turrets.length === m.turrets.length) {
				for (var _i13 = 0; _i13 < m.turrets.length; _i13++) {
					...
					if (_t.layer === 1) {
						...
						drawEntity(xx + _len6 * Math.cos(_ang), yy + _len6 * Math.sin(_ang), _t, ratio, 1, drawSize / ratio / _t.size * _t.sizeFactor, source.turrets[_i13].facing + turretsObeyRot * rot, turretsObeyRot, context, source.turrets[_i13], render);
					}
				}
			} else {
				throw new Error("Mismatch turret number with mockup.");
		...
		return function (ratio) {
...

			{
				...
				for (var _i14 = messages.length - 1; _i14 >= 0; _i14--) {
					...
					drawBar(_x30 - msg.len / 2, _x30 + msg.len / 2, _y3 + height / 2, height, color.black);
					// Draw the text
					ctx.globalAlpha = Math.min(1, msg.alpha);
					msg.textobj.draw(_text, _x30, _y3 + height / 2, height - 4, color.guiwhite, 'center', true);
					// Iterate and move
					_y3 += vspacing + height;
					if (msg.status > 1) {
						_y3 -= (vspacing + height) * (1 - Math.sqrt(msg.alpha));
					}
					if (msg.status > 1) {
						msg.status -= 0.05;
						msg.alpha += 0.05;
...

			{
				...
				var save = _len7;
				var _x31 = -spacing - 2 * _len7 + statMenu.get() * (2 * spacing + 2 * _len7);
				var _y4 = global.screenHeight - spacing - _height;
				var ticker = 11;
				...
				_gui.skills.forEach(function drawASkillBar(skill) {
					...
					if (cap) {
						...
						drawBar(_x31 + _height / 2, _x31 - _height / 2 + _len7 * ska(cap), _y4 + _height / 2, _height - 3 + config.graphical.barChunk, color.black);
						drawBar(_x31 + _height / 2, _x31 + _height / 2 + (_len7 - gap) * ska(cap), _y4 + _height / 2, _height - 3, color.grey);
						drawBar(_x31 + _height / 2, _x31 + _height / 2 + (_len7 - gap) * ska(level), _y4 + _height / 2, _height - 3.5, col);
						// Blocked-off area
						if (blocking) {
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x31 + (_len7 - gap) * ska(j), _y4 + 1.5, _x31 + (_len7 - gap) * ska(j), _y4 - 3 + _height);
							}
						}
						// Vertical dividers
						...
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x31 + (_len7 - gap) * ska(_j), _y4 + 1.5, _x31 + (_len7 - gap) * ska(_j), _y4 - 3 + _height);
						}
						// Skill name
						_len7 = save * ska(_max);
						var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
						text.skillNames[ticker - 1].draw(name, Math.round(_x31 + _len7 / 2) + 0.5, _y4 + _height / 2, _height - 5, textcolor, 'center', true);
						// Skill key
						text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x31 + _len7 - _height * 0.25) - 1.5, _y4 + _height / 2, _height - 5, textcolor, 'right', true);
						if (textcolor === color.guiwhite) {
							// If it's active
							global.clickables.stat.place(ticker - 1, _x31, _y4, _len7, _height);
						}
						// Skill value
						if (level) {
							text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x31 + _len7 + 4) + 0.5, _y4 + _height / 2, _height - 5, col, 'left', true);
						}
						// Move on 
						_y4 -= _height + _vspacing;
					}
				});
				global.clickables.hover.place(0, 0, _y4, 0.8 * _len7, 0.8 * (global.screenHeight - _y4));
				if (_gui.points !== 0) {
					// Draw skillpoints to spend
					text.skillPoints.draw('x' + _gui.points, Math.round(_x31 + _len7 - 2) + 0.5, Math.round(_y4 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
				}
			}

			{
				...
				var _height2 = 25;
				var _x32 = (global.screenWidth - _len8) / 2;
				var _y5 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				// Handle exp
				// Draw the exp bar
				drawBar(_x32, _x32 + _len8, _y5 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x32, _x32 + _len8, _y5 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x32, _x32 + _len8 * _gui.__s.getProgress(), _y5 + _height2 / 2, _height2 - 3.5, color.gold);
				// Draw the class type
				text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x32 + _len8 / 2, _y5 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', true);
				_height2 = 14;
				_y5 -= _height2 + _vspacing2;
				// Draw the %-of-leader bar
				drawBar(_x32 + _len8 * 0.1, _x32 + _len8 * 0.9, _y5 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x32 + _len8 * 0.1, _x32 + _len8 * 0.9, _y5 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x32 + _len8 * 0.1, _x32 + _len8 * (0.1 + 0.8 * (max ? Math.min(1, _gui.__s.getScore() / max) : 1)), _y5 + _height2 / 2, _height2 - 3.5, color.green);
				// Draw the score
				text.score.draw('Score: ' + handleLargeNumber(_gui.__s.getScore()), _x32 + _len8 / 2, _y5 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', true);
				// Draw the name
				ctx.lineWidth = 4;
				text.name.draw(player.name, Math.round(_x32 + _len8 / 2) + 0.5, Math.round(_y5 - 10 - _vspacing2) + 0.5, 32, color.guiwhite, 'center');
			}

			{
				...
				var _height3 = _len9;
				var _x33 = global.screenWidth - _len9 - spacing;
				var _y6 = global.screenHeight - _height3 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x33 + j++ * _len9 / _W, _y6 + _i15 * _height3 / _H, _len9 / _W, _height3 / _H);
					});
					_i15++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x33, _y6, _len9, _height3);
				minimap.forEach(function (o) {
					if (o[2] === 17) {
						ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.globalAlpha = 0.8;
						drawGuiRect(_x33 + o[0] / global.gameWidth * _len9, _y6 + o[1] / global.gameHeight * _height3, 1, 1);
					} else {
						ctx.strokeStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.lineWidth = 1;
						ctx.globalAlpha = 1;
						drawGuiRect(_x33 + o[0] / global.gameWidth * _len9 - 1, _y6 + o[1] / global.gameWidth * _height3 - 1, 3, 3, true);
						ctx.lineWidth = 3;
					}
				});
				ctx.globalAlpha = 1;
				...
				_x33 + player.x / global.gameWidth * _len9 - 1, _y6 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				ctx.fillStyle = color.black;
				drawGuiRect(_x33, _y6, _len9, _height3, true); // Border

				drawGuiRect(_x33, _y6 - 40, _len9, 30);
				lagGraph(lag.get(), _x33, _y6 - 40, _len9, 30, color.teal);
				gapGraph(metrics.rendergap, _x33, _y6 - 40, _len9, 30, color.pink);
				timingGraph(GRAPHDATA, _x33, _y6 - 40, _len9, 30, color.yellow);
				// Text
				text.debug[5].draw('Prediction: ' + Math.round(GRAPHDATA * 10) / 10 + 'ms', _x33 + _len9, _y6 - 50 - 5 * 14, 10, color.guiwhite, 'right');
				text.debug[4].draw('Update Rate: ' + metrics.updatetime + 'Hz', _x33 + _len9, _y6 - 50 - 4 * 14, 10, color.guiwhite, 'right');
				text.debug[3].draw('Latency: ' + metrics.latency + 'ms', _x33 + _len9, _y6 - 50 - 3 * 14, 10, color.guiwhite, 'right');
				text.debug[2].draw('Client FPS: ' + metrics.rendertime, _x33 + _len9, _y6 - 50 - 2 * 14, 10, color.guiwhite, 'right');
				text.debug[1].draw('Server Speed: ' + (100 * _gui.fps).toFixed(2) + '%' + (_gui.fps === 1 ? '' : ' OVERLOADED!'), _x33 + _len9, _y6 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
				text.debug[0].draw(serverName, _x33 + _len9, _y6 - 50, 10, color.guiwhite, 'right');
			}

			{
				...
				var _height4 = 14;
				var _x34 = global.screenWidth - _len10 - spacing;
				var _y7 = spacing + _height4 + 7;
				text.lbtitle.draw('Leaderboard:', Math.round(_x34 + _len10 / 2) + 0.5, Math.round(_y7 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i16 = 0;
				lb.data.forEach(function (entry) {
					drawBar(_x34, _x34 + _len10, _y7 + _height4 / 2, _height4 - 3 + config.graphical.barChunk, color.black);
					drawBar(_x34, _x34 + _len10, _y7 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x34, _x34 + _len10 * shift, _y7 + _height4 / 2, _height4 - 3.5, entry.barcolor);
					// Leadboard name + score 
					text.leaderboard[_i16++].draw(entry.label + ': ' + handleLargeNumber(Math.round(entry.score)), _x34 + _len10 / 2, _y7 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', true);
					// Mini-image
					var scale = _height4 / entry.position.axis,
					    xx = _x34 - 1.5 * _height4 - scale * entry.position.middle.x * 0.707,
					    yy = _y7 + 0.5 * _height4 + scale * entry.position.middle.x * 0.707;
					drawEntity(xx, yy, entry.image, 1 / scale, 1, scale * scale / entry.image.size, -Math.PI / 4, true);
					// Move down
					_y7 += _vspacing3 + _height4;
				});
			}
			...
			//Draw upgrade menu
			{
				...
				if (_gui.upgrades.length > 0) {
					...
					var _height5 = _len11;
					var _x35 = glide * 2 * spacing - spacing;
					var _y8 = spacing;
					var xo = _x35;
					var xxx = 0;
					var yo = _y8;
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y8 > yo) yo = _y8;
						xxx = _x35;
						global.clickables.upgrade.place(_i17++, _x35, _y8, _len11, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x35, _y8, _len11, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x35, _y8, _len11, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x35, _y8 + _height5 * 0.6, _len11, _height5 * 0.4);
						ctx.globalAlpha = 0.9;
						...
						var picture = getEntityImageFromMockup(model, _gui.color),
						    ...
						    xx = _x35 + 0.5 * _len11 - scale * position.middle.x * Math.cos(upgradeSpin),
						    yy = _y8 + 0.5 * _height5 - scale * position.middle.x * Math.sin(upgradeSpin);
						drawEntity(xx, yy, picture, 1, 1, scale / picture.size, upgradeSpin, true);
						// Tank name
						text.upgradeNames[_i17 - 1].draw(picture.name, _x35 + _len11 / 2, _y8 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x35, _y8, _len11, _height5, true); // Border
						if (_ticker++ % 2) {
							_y8 -= _height5 + internalSpacing;
							_x35 += glide * (_len11 + internalSpacing);
						} else {
							_y8 += _height5 + internalSpacing;
						}
					});
